...
Personal developer page as a monochrome phosphor terminal with a single green accent. A tiled hacker desktop with a prompt you can actually type into. Static, no build step, no dependencies, no frameworks.
...
Styling splits across `styles/`: `tokens.css` holds every colour and size, `layout.css` the pane grid, `terminal.css` the prompt and output blocks, `crt.css` the scanlines and glow. Full phosphor `#ffffff` is reserved for the prompt, headings and accents; body copy uses a dimmed grey so a screen of project text stays readable. A muted green `#63b17f` is spent only on what is alive or actionable: the caret and focus ring, the title-bar LED, the `live` badge, directory entries and hover states. Red `#ff4d4d` is reserved for errors. Everything else stays grey, which is what keeps the green worth looking at. The one deliberate exception is the stack tags on a project page: each tool wears its own brand colour, because a stack list is a reference grid where recognition beats restraint. Every tag colour is brightened as needed to clear 4.5:1 on the background. Warn output is additionally marked `[!]` in text, errors `[x]`, so status never depends on hue alone.
